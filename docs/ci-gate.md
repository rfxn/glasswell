...
The nightly run above is the whole suite, and it is still a statement about the tree. CI runs
on GitHub-hosted runners: they have no route to the deployed instance, no owner key and no
host filesystem, so **neither `ci.yml` nor `nightly.yml` asserts anything about a running
deployment**, and neither can be given the probes without giving a runner the credential.
