...
Every figure below was measured on **2026-09-01** and names where; nothing is carried forward
from a prior revision. [`ROADMAP.md`](ROADMAP.md) owns phase scope and exit criteria;
...
| **NM** 30 | 321,510 / 142,000 | 17,597,960 · `nm_ocd_wcproduction`, completion-pool grain | surface 141,778 | **0 of 321,510** — `cr_nm_wellhistory_status_vocab_1` maps nothing; resolution lands in v0.74 | lateral geometry `data-unreachable` · `cr_nm_wellhistory_geometry_scope_1`. No well-level rollup · `cr_nm_wcproduction_pool_rollup_1` |
...
Explore and the map. Basins and plays are served tile layers with no layer-registry row in
front of them, so the Geology group presents as pending while the data is live.
...
Each item names the track that carries it in `work-output/po-review/PRODUCT-REVIEW.md` §5,
or the horizon in §4.

1. **NM status resolution** — 321,510 NM wells carry no status class. v0.74, `feat/nm-status-resolution`.
2. **Map→Explore crossing** writes the API-10 into a well-name filter and returns zero rows;
   the test asserts the bug. v0.74, `feat/map-truth`.
3. **Basins and plays** are served and unreachable — no layer row, and two placeholder rows
   that say "no ingest recipe yet". v0.74, `feat/map-truth`.
4. **N2 re-land** — 17 gated commits on `feat/n2-enrich-views` never merged. v0.75, `feat/n2-reland`.
...
9. **Texas production** — the largest resident state has no production number. H2 (v0.78).
10. **P6 residuals** — a restore proof at the current schema, replacement-host recovery
    execution, and an off-LAN credential exercise. H3.
11. **P3–P5 modeling and economics**, sequenced after the registry lands. H3. The owner-gated
