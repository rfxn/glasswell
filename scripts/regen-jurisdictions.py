...
Three artifacts, because three readers need different things. `jurisdictions.generated.ts` is
the typed module the bundle imports; `wells-roster.json` is the same wells rows as data, so
`tests/e2e/chrome-fold.mjs` -- plain node ESM with no TypeScript loader -- can read the roster
it asserts over instead of parsing a module as text; and `status-classes.generated.ts` is the
status class domain as a **test fixture only**, so the vitest that asserts what the map renders
compares it against the seed rather than against a hand-written copy of it.

The domain is deliberately not generated for the bundle. It carries a rule id and an effective
date, a constant can carry neither, and a second shipped copy is the defect the domain exists to
close: the client fetches it from `/v1/jurisdictions` and `status-vocabulary.test.ts` refuses any
shipped module that imports the fixture.
...
DOMAIN_HEADER = """// Generated by scripts/regen-jurisdictions.py from
...
def domain() -> str:
    """The twelve rows as the wire serves them, for the parity gate and for nothing else."""
    rows = ""
    for row in sorted(
        (class_parameters(item) for item in STATUS_CLASSES),
        key=lambda item: int(item["sort_order"]),  # type: ignore[arg-type]
    ):
        rows += (
            "  {\n"
            f"    status_canonical: {json.dumps(row['status_canonical'])},\n"
            f"    label: {json.dumps(row['label'])},\n"
            f"    colour: {json.dumps(row['colour'])},\n"
            f"    glyph: {json.dumps(row['glyph'])},\n"
            f"    min_zoom: {int(row['min_zoom'])},\n"  # type: ignore[arg-type]
            f"    sort_order: {int(row['sort_order'])},\n"  # type: ignore[arg-type]
            f"    is_absence: {'true' if row['is_absence'] else 'false'},\n"
            f"    note: {json.dumps(row['note'])},\n"
            f"    rule_id: {json.dumps(row['rule_id'])},\n"
            "  },\n"
        )
    return DOMAIN_HEADER + rows + "];\n"
...
def main(argv: list[str] | None = None) -> int:
    ...
    domain_target = arguments.target.parent / DOMAIN.name
    artifacts = (
        (arguments.target, rendered()),
