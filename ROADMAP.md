...
40 tagged releases, v0.20 through v0.59, cut from 2026-08-21 through 2026-08-28, run
...
| **P3** Forecasting and benchmark | **Pinned control and publication gate built; resident gate red.** The immutable resident `fv2.0`, `mdv1.4`, and `tcv1.0` artifacts retain their published identities and 12.9484% control unavailability. A rollback-only same-manifest repair rehearsal restores all 318 TEST formation gaps without inferring any of the 38 source-absent laterals, preserves all eight exact split hashes, replays unchanged `tcv1.0` byte-identically, and measures 1.0798% unavailability (230 / 21,300). A fail-closed publisher now pins deployed code and dependency-lock identity, requires two byte-identical builds and persists an immutable family receipt. Live publication still waits for deployment and a new evaluation vintage. No model, calibration, analog index or benchmark runner exists, and the model registry remains DDL with no writer |
...
| **P6** Hardening and glass-box proof | **Partial.** Six CI jobs are wired and blocking — `python`, `web`, `e2e-guards`, `shell`, `collateral`, `map-chrome` — with the tile allowlist asserted against the served source list and the conformance registry exercised end to end. A mandatory 15-minute sanitized snapshot powers Status with core service probes, storage guardrails, job observations, exact-grain dataset counts, all-source registered-artifact age and explicit limits. Nightly logical dumps share an exported snapshot with strict sidecar manifests; the exact corrected weekly-restore sandbox has passed live with schema, four critical counts, six representative reads, durable evidence and scratch cleanup all verified. The tagged recurring unit still needs deployment. Completion and neighbour selector-output checks are strict but not yet universal. Full VM/raw-zone recovery remains open, as do durable source attempts/cadence, independent rule publication time, tunnel/Access, outsider guest exercise, determinism and tool-equivalence gates |
