...
| **P6** Hardening and glass-box proof | **Partial.** Six CI jobs are wired and blocking — `python`, `web`, `e2e-guards`, `shell`, `collateral`, `map-chrome` — with the tile allowlist asserted against the served source list and the conformance registry exercised end to end. Hosted run `32756994380` passed all six on 2026-08-24 after the stale tile-contract selector was corrected. HTTPS on the LAN name; key issue, rotation and revocation served and audited; all three scopes in code; a restore-drill script written. Open: no tunnel and no Access, no guest key exercised by an outsider, no recorded live drill, and no determinism or tool-equivalence gate |
