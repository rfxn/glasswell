...
33 tagged releases, v0.20 through v0.52, cut from 2026-08-21 through 2026-08-26, run
...
| **P3** Forecasting and benchmark | **Matrix integrity built; resident replay pending.** Canonical has 131 distinct production months, earliest 2015-05-01, against the entry gate. Immutable `fv1.0` and semantic-major `fv2.0` are declared; `fv2.0` freezes the earliest source pool month, emits simultaneous conflicts as null, publishes missing/conflict and anchor-timing coverage, records the measured 82-day median publication lag, and distinguishes strict history from reconstructed source availability. Deterministic pad grouping, grouped temporal split, and benchmark schema are implemented. The live `fv2.0` replay, model-ready dataset, and type-curve control are next; no model, calibration, analog index or benchmark runner exists, and the model registry remains DDL with no writer |
...
| **P6** Hardening and glass-box proof | **Partial.** Six CI jobs are wired and blocking — `python`, `web`, `e2e-guards`, `shell`, `collateral`, `map-chrome` — with the tile allowlist asserted against the served source list and the conformance registry exercised end to end. Hosted head run `32779964976` passed all six on 2026-08-24 after the P3 foundation merge, with zero annotations. HTTPS on the LAN name; key issue, rotation and revocation served and audited; all three scopes in code; a restore-drill script written. Open: no tunnel and no Access, no guest key exercised by an outsider, no recorded live drill, and no determinism or tool-equivalence gate |
