...
| **P6** Hardening and glass-box proof | **Partial.** Six CI jobs are wired and blocking — `python`, `web`, `e2e-guards`, `shell`, `collateral`, `map-chrome` — with the tile allowlist asserted against the served source list and the conformance registry exercised end to end. Hosted PR run `33013817153` passed all six on 2026-08-26 at implementation-review head `978e2fc`. HTTPS on the LAN name; key issue, rotation and revocation served and audited; all three scopes in code; a restore-drill script written. Open: `/explain` resolves a handle's derivation but does not enforce selector cardinality; conformance rules carry valid time but no independent knowledge-publication date; no tunnel/Access, outsider guest exercise, recorded live drill, determinism gate, or tool-equivalence gate |
