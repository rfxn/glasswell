...
| **P3** Forecasting and benchmark | **Model-ready dataset built and replayed.** Canonical has 131 distinct production months from 2015-05-01. Immutable `fv1.0` and semantic-major `fv2.0` are declared; the resident `fv2.0` matrix and `mdv1.2` dataset replay byte-identically. The dataset persists 105,378 three-stream labels, 1,172,586 observed curve rows, explicit rejection/coverage artifacts, and eight content-addressed rolling splits over 15,749 eligible wells. E-6 measures the intermittency guard at 16 months. The pinned type-curve control is next; no model, calibration, analog index or benchmark runner exists, and the model registry remains DDL with no writer |
