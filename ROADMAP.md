...
31 tagged releases, v0.20 through v0.50, cut from 2026-08-21 through 2026-08-26, run
...
| **P3** Forecasting and benchmark | **Data seam resident; first matrix pending.** Canonical has 131 distinct production months, earliest 2015-05-01, against the entry gate. The first `fv1.0` declaration, C1/C2 D1 feature materializer, deterministic pad grouping, grouped temporal split, and benchmark schema are implemented. The 2026-08-26 resident load assigns all 43,817 ND wells to Williston and retains 18,665 valid FracFocus events across 17,563 API-10s; the 26,254 uncovered wells remain null. Single-pool completion entities and 40 reviewed pool aliases are resident. The first live matrix remains the next proof point. No type-curve control, model, calibration, analog index or benchmark runner; the model registry remains DDL with no writer |
