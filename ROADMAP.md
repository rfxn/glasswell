...
| **P3** Forecasting and benchmark | **Foundation advanced.** Canonical has 131 distinct production months, earliest 2015-05-01, against the ≥ 120 and ≤ 2016-01 gate. The first `fv1.0` declaration, C1/C2 feature materializer, D1 content-addressed feature-matrix artifact, deterministic 150 m / 180 day pad grouping, grouped temporal split, and strict benchmark artifact schema are implemented and tested. The materializer correctly emits no live ND partition yet: all 43,817 current ND well rows lack `completion_date`, and no conformed formation alias is resident. No type-curve control, model, calibration, analog index or benchmark runner; the model registry remains DDL with no writer |
