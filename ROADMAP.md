...
35 tagged releases, v0.20 through v0.54, cut from 2026-08-21 through 2026-08-26, run
...
| **P2** Serving and map | **Substantially met.** Thirty-three snapshot-pinned operations, thirty-two under `/v1`; tiles from PostGIS with the layer allowlist asserted in CI; URL-backed Map, Explore, and Status surfaces; well card, lineage drawer and glossary tooltip shipped; the frame-rate budget codified in the perf harness. Source-observed completion events and pool-to-formation mappings are separate API/card sections, and canonical formations are served with current alias counts. Completion-design measurements remain unpromoted rather than read from staging. Neighbours, permits, land units, spacing units, GOR and water-cut remain |
| **P3** Forecasting and benchmark | **Pinned type-curve implementation built; resident gate red, repair rehearsal green.** The immutable resident `fv2.0`, `mdv1.4`, and `tcv1.0` artifacts retain their published identities and 12.9484% control unavailability. A rollback-only same-manifest repair rehearsal restores all 318 TEST formation gaps without inferring any of the 38 source-absent laterals, preserves all eight exact split hashes, replays unchanged `tcv1.0` byte-identically, and measures 1.0798% unavailability (230 / 21,300). Publication waits for deployment and a new evaluation vintage. No model, calibration, analog index or benchmark runner exists, and the model registry remains DDL with no writer |
...
| **P6** Hardening and glass-box proof | **Partial.** Six CI jobs are wired and blocking — `python`, `web`, `e2e-guards`, `shell`, `collateral`, `map-chrome` — with the tile allowlist asserted against the served source list and the conformance registry exercised end to end. A mandatory 15-minute sanitized snapshot now powers the Status surface with core service probes, storage guardrails, job observations, exact-grain dataset counts, all-source registered-artifact age, and explicit observability limits. HTTPS on the LAN name; key issue, rotation and revocation served and audited; all three scopes in code; a restore-drill script written. Open: durable unchanged/failed fetch attempts and source-specific cadence; `/explain` selector cardinality; independent conformance-rule publication time; tunnel/Access, outsider guest exercise, recorded live drill, determinism gate, and tool-equivalence gate |
