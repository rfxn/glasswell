...
| **P3** Forecasting and benchmark | **Pinned type-curve implementation built; coverage gate red.** The resident `fv2.0` matrix, `mdv1.4` model bundle, and resident-scale `tcv1.0` control candidate replay byte-identically. The control emits 2,300,400 rows for three streams, two normalization arms, and all eight exact splits; rung-one coverage is 81.7089%, but 12.9484% of TEST subject/split instances are unavailable against the pinned 5% ceiling. The miss is context, not peer count: 318 unique TEST subjects lack formation, 38 lack lateral length, and only one lacks enough peers. No model, calibration, analog index or benchmark runner exists, and the model registry remains DDL with no writer |
