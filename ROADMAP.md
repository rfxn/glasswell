...
24 tagged releases, v0.20 through v0.43, cut on 2026-08-21 and 2026-08-22, run a
North Dakota slice on one VM. Status is per phase and stated against the exit
...
| **P1** ND spine | **Met but for the production history.** Ingest, promotion with conformance references and bitemporal vintages, and a live quarantine with a measured share, across the monthly production report, the DMR GIS layers and the PLSS grid. Two gaps: the PDF era is deferred inside this phase by design; the XLSX era is loaded for six months only (2025-10 → 2026-03), and that back-load is P3's entry gate rather than a P1 deferral. No FracFocus ingest, which P1's contents name |
| **P2** Serving and map | **Substantially met.** Thirty snapshot-pinned operations, twenty-nine under `/v1`; tiles from PostGIS with the layer allowlist asserted in CI; map, well card, lineage drawer and glossary tooltip shipped; the frame-rate budget codified in the perf harness. Completions, neighbours, permits, land units, spacing units and formations are named in P2's contents and are not routes, though several have tables and marts behind them. No GOR or water-cut chart on the card |
| **P3** Forecasting and benchmark | **Not started, and gated at the door.** Entry needs ≥ 120 distinct production months with the earliest at or before 2016-01; six are resident. No feature builder, model, calibration or harness; the model registry is DDL with no writer |
...
| **P6** Hardening and glass-box proof | **Partial.** Five CI tiers wired and blocking — `python`, `web`, `e2e-guards`, `shell`, `collateral` — with the tile allowlist asserted against the served source list and the conformance registry exercised end to end. HTTPS on the LAN name; key issue, rotation and revocation served and audited; all three scopes in code; a restore-drill script written. Open: no tunnel and no Access, so delivery is LAN-only; no guest key exercised by an outsider; no drill run and recorded; no determinism or tool-equivalence check |
...
| **P3** Forecasting and benchmark | Entry is the production back-load, without which not one rolling origin is reachable. The contract's P3 row gives its span as 2015-05 → 2025-09 and its size as 134 monthly workbooks; those disagree — that span is 125 months — and the discrepancy is open against the contract, not resolved here. Feature builder with declared availability dates; type-curve baseline; three-stream gradient-boosted quantile models; split-conformal calibration; model registry; persisted analog index; benchmark harness | Entry gate: ≥ 120 distinct production months in canonical with the earliest at or before 2016-01. Then calibration green and the analog check measured against its bracket rather than asserted — an inconclusive interval blocks exit exactly as a failure does; the pinned type-curve control on the identical split; per-slice empirical coverage published with its interval; the determinism check green; every forecast citing a registered artifact, and no number served from an unregistered one |
...
geology · additional basins · TX inventory geometry · public release (IP-gated, see
...
| Risk | Mitigation |
|------|------------|
| **IP carve-out** (top item, time-sensitive) | Now schedulable rather than open-ended: the trigger is the first public URL or the first published matrix row, and the review completes before P8 exits. Until then every artifact stays private and the guest scope serves invited outsiders only |
...
| **Source access drift** — identifiers rotate without notice, page structure changes | Identifier resolution is a monitored step with its own job and alert; every fetch writes a log row, so "nothing changed" and "nothing fetched" stay distinguishable; health goes degraded on a missed pull window |
