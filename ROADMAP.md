...
| **P3** Forecasting and benchmark | **Foundation started.** Canonical has 131 distinct production months, earliest 2015-05-01, against the ≥ 120 and ≤ 2016-01 gate. `features.feature_specs`, the C1 availability guard, deterministic 150 m / 180 day pad grouping, the grouped temporal split, and the strict content-addressed benchmark artifact schema are implemented and tested. No feature computation, type-curve control, model, calibration, analog index or benchmark runner; the model registry remains DDL with no writer |
...
| **P6** Hardening and glass-box proof | **Partial.** Six CI jobs are wired and blocking — `python`, `web`, `e2e-guards`, `shell`, `collateral`, `map-chrome` — with the tile allowlist asserted against the served source list and the conformance registry exercised end to end. Hosted head run `32758605990` passed all six on 2026-08-24 after the stale tile-contract selector was corrected. HTTPS on the LAN name; key issue, rotation and revocation served and audited; all three scopes in code; a restore-drill script written. Open: no tunnel and no Access, no guest key exercised by an outsider, no recorded live drill, and no determinism or tool-equivalence gate |
