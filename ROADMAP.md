...
| **P6** Hardening and glass-box proof | **Partial.** Six CI jobs are branch-protection-required — `python`, `web`, `e2e-guards`, `shell`, `collateral`, `map-chrome` — with the tile allowlist asserted and conformance exercised end to end. The v0.60 candidate locally proves independently committed source-poll outcomes aligned to actual recurring timers, fail-closed selector outputs, two-clock conformance, and a sandboxed nightly retention unit; hosted and deployed verification remain release gates. The recurring logical restore has passed live against the latest schema-47 backup with exact critical counts, six reads, durable evidence and scratch cleanup. Full VM/raw-zone recovery remains open, as do rate limits, remote-copy evidence, tunnel/Access, outsider guest exercise, broader determinism and tool-equivalence gates |
