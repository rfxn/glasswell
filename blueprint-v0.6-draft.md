...
- `canonical.well_completions` — the entity behind `entity_type = well_completion_pool`: `completion_pool_key` (PK, the OCD completion key), `state`, `pool_id`, `api10s[]` (the member wells), `effective_from`, `effective_to`, `source_id`, `manifest_id`, `rule_ids[]`. Summing pools to a well is exact arithmetic on observations, so it stays `granularity = observed` with `aggregation = sum_over_pools` recorded in the derivation params and surfaced (R5).
...
- `models` — `model_id` (`mdl_<sha256[:32]>`, content-addressed), `name`, `kind` (`quantile_member` | `cqr_bundle`), `target_stream`, `quantile` (member rows only), `horizon`, `basin`, `algo`, `algo_version`, `hyperparams`, `feature_version`, `training_data_vintage`, `training_cutoff`, `artifact_uri`, `artifact_sha256`, `seed`, `lockfile_sha256`, `member_model_ids[]` (bundle rows), `calibration_ref`, `promotion_status` (candidate | published | shadow | retired), `supersedes_model_id`, `created_at`. **A forecast cites a bundle, not a quantile row.** One `cqr_bundle` row exists per `(stream, horizon)`, content-addressed over its member model ids, the calibrator artifact hash, `feature_set_hash`, `conformal_alpha` and the split/holdout taxonomy; the three quantile members stay registered and are referenced by it. Without the bundle a three-quantile forecast maps to three rows while 4A.13 requires it to cite *a* `model_id`, so the citation is ambiguous. Content-addressing beats a ULID here for the reason SB-07 wanted a ULID — identical inputs yield an identical id by construction — and `promotion_status` plus the timestamps stay mutable, because the id addresses the artifact, not its lifecycle. Models are never deleted; retraining is additive; nothing serves from a non-promoted model except a row explicitly flagged `shadow`. *(Resolves assessment A-01.)*
- `calibration_sets` — `calibration_id`, `model_id`, `split_definition`, `calibrator_artifact_uri` + `calibrator_artifact_sha256` (the fitted `Q_lo`/`Q_hi` per group — the calibrator is an artifact, distinct from the coverage *report* about it), `empirical_coverage_by_slice` (jsonb), `coverage_ci_by_slice` (jsonb), `derivation_id`.
...
- `forecasts` — `forecast_id` (content-addressed), `api10` or `scenario_id`, `model_id` (the bundle), `feature_version`, `as_of_vintage`, `stream`, `horizon_months`, `p10`/`p50`/`p90` series, `quantile_convention` (statistical-ascending; mandatory, 4A.7), `granularity` = `modelled`, `method`, `training_support` with its `k`, `metric` and `d_ref`, `derivation_id`, `created_at`.
...
- **Naked-number check.** Walk the OpenAPI, exercise every GET with fixture parameters, JSON-walk every numeric leaf of `data`, and assert it is a figure object carrying `d` and `unit` or is covered by a `_lineage`/`_units` sidecar (§3.6.2). Failure fails the build. *(Resolves assessment A-09.)*
