...
- **Tile-layer allowlist check (S-C).** martin's configured source list must equal the checked-in allowlist file, exactly. This is the entitlement mechanism for tiles, so it is a build gate rather than a review habit.
