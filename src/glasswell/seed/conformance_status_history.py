...
The rule is one row and the registry decides who it answers for: it registers as the
`status_history` decision for exactly the jurisdictions whose header carries a source valid
time, so a fifth jurisdiction joins by registering rather than by a code change.
"""
...
HISTORY_RULE_ID = "cr_status_history_basis_1"
...
HISTORY_RULES: tuple[dict[str, object], ...] = (
    {
        ...
        "rationale": (
            "Measured on the deployed spine on 2026-09-02. Zero of 585,864 wells carry more"
            " than one distinct status_canonical, so a canonical timeline would render empty"
            " for every well in the system; 31,707 wells carry a changed status_reported and"
            " every one of them is in New Mexico. New Mexico is the only jurisdiction in"
            " production whose header clock is a source valid time: it holds 15,590 distinct"
            " effective_from values spanning 1900-01-01 to 2026-08-19 against North Dakota's"
            " two, Montana's one and Texas's one, because cr_nm_wellhistory_effective_1"
            " promotes eff_dte and the other three promote the vintage of the extract the"
            " header came from. Colorado registers the same shape without yet holding wells in"
            " the spine: cr_co_wells_effective_1 promotes Stat_Date, the date ECMC stamped the"
            " status, and states in its own rationale that keying on the pull would append"
            " 124,392 rows a night and make the spine a log of when glasswell looked. So this"
            " rule registers for New Mexico and Colorado and for nobody else, and the three"
            " load-stamp jurisdictions serve the absence with their own status_vocabulary rule"
            " beside it."
        ...
        "evidence_url": (
            "https://ocdimage.emnrd.nm.gov/imaging/OCDPermitsData/wellhistory.zip"
        ),
