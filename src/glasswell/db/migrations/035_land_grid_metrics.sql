...
select 'cr_land_agg_membership_1', 'cr_land_agg_membership', 'blm_plss_sections', 'join',
       ...
       jsonb_build_object(
           ...
           'evidence_measured', jsonb_build_object(
               ...
               'multi_lateral_wells', jsonb_build_object(
                   'count', 695, 'with_differing_midpoints', 256),
               'midpoint_orphans', jsonb_build_object(
       ...
       ' bottomhole geometries (the 360,434 on file are all Texas). Surface-point'
       ...
       ' 57.33% of every observed ND liquid barrel (107.78M of 187.98M, well-entity rows'
       ' only), because pads cluster surface holes in one section while the rock that'
       ' produced sits under the'
