...
Owner: Ryan MacDonald. Status: in build — four states are deployed and serving; `STATUS.md` carries every count and its date. This document is the contract to spec against; anything not in scope here is out until this doc changes.
...
- **[as-built 2026-09-02]** **Five states are resident**, and this section scoped three. **CO (Colorado)** is the first of the Rockies sequence below and the first jurisdiction admitted the way 3.0.1a says a jurisdiction should be: a `lineage.jurisdictions` row, its `jurisdiction_rules` children and an R8 rule set with published ECMC evidence, with no edit to the routers, the legend census constants or the status collector. Its named risk was measured rather than inherited -- the status codebook is not materially incomplete, because the shapefile's in-band legend is the stale one and the live reference list publishes every code in the data -- and the codes with no canonical counterpart are `documented_unmapped` rows rather than lookups. It promotes surface points only, its production is per completion with a summed well row beside it, and inventory is a **registered refusal**, because Protocol 4D admits no slot without a spacing assumption and a support score and Colorado has neither. **ND** is end to end. **TX** carries wells, wellbore identity and bore geometry and **no production**: RRC PDQ lease production is not a registered source, so allocation v0 and both validators are still unbuilt. **NM** carries headers, surface geometry and Tier 1 production at the completion-pool grain. **MT (Montana)** was never in scope here and is now resident on both production grains, with tiles and well paths. Montana is a **Williston extension, not a third basin**: it is the same Bakken already trained on, and MBOGC publishes bulk well-level *and* lease/unit-level production from one regulator, which makes the well-level file a control for the lease-level file and rehearses the TX allocation problem against ground truth before allocation v0 is written. It renumbers no phase; ROADMAP's N3 owns its exit criteria. Every count and the date it was measured live in `STATUS.md`, never here.
...
**Deferred until after P6:** Canada, NGL three-stream economics beyond simple gas pricing, fault-aware geology, additional basins beyond the Rockies sequence named below, public release (IP-gated).

**In scope under the registry: the Rockies sequence (Colorado, Utah, Wyoming).** These three enter as registrations rather than as projects (3.0.1a): each is one `lineage.sources` row, one jurisdiction row and its R8 rule set, over promotion, inventory and serving paths that already read the registry rather than an API-10 prefix. That is Montana's argument in the shape the registry gives it. Montana was admitted because it extends a basin already trained on, so it needed no new analogues; the Rockies sequence is admitted because it needs no new serving path, which makes a state a registration cost rather than a phase. Each carries its own reachability evidence and its own named risk, and neither is assumed. **Colorado (ECMC)** publishes headers, monthly production and lateral geometry as free static files at per-completion grain, so no allocation step is required, and states its datum in-band in the `.prj`; its named risk is a materially incomplete status codebook, so every undocumented code is a `conformance_rules` row with a written rationale rather than a lookup. **Utah (DOGM)** is the only reachable source publishing per-station directional surveys in bulk with latitude and longitude already computed, which is what Protocol 4D geometric admissibility needs and neither ND nor TX supplies; its named risk is a bidirectional grain mismatch between an API-14 header and a five-part production key, which is a conformance rule before a single row lands. **Wyoming (WOGCC)** carries surface and bottom-hole coordinates for its whole well population in one file alongside production from 1978; its named risk is a datum the regulator declares nowhere and that its own downstream users read two different ways, booked as a conformance rule with a stated assumption and never as a code default. None of the three renumbers a phase; ROADMAP's H2 owns their sequencing, and every count and the date it was measured lives in `STATUS.md`, never here.
...
**3.0.1a Jurisdiction as a registered layer boundary.** A state is a **registration, not a project**. A jurisdiction is one `lineage.sources` row, one jurisdiction row, and its R8 rule set — status vocabulary, geometry provenance, liquids basis, production grain, unmapped action — plus an ingest module that writes staging and nothing else. The **promotion, inventory and serving paths read the jurisdiction registry**; no path may branch on an API-10 prefix. A prefix is an identifier and a jurisdiction is a decision, and a decision that lives only in code cannot be cited by the derivations it shaped, which is exactly what R8 forbids. **Status is resolved once, not per surface:** the resolution is a single canonical-layer relation over the registry's per-jurisdiction code maps (`canonical.status_resolution`, a view since v0.74 and a keyed table behind that view since v0.78, so a join on both keys is an index lookup), and every serving path — the tile mart and the API alike — joins the spine onto that one relation through one shared helper. The class domain the maps target is itself registered (`lineage.status_classes`), so a map cannot introduce a class no rule declared. No surface carries a mapping of its own. Tiles, the well card and the legend reach the same wells through different code paths, so a translation applied at any one of them lets them disagree on screen with nothing on the page to reconcile them. A regulator code that is **documented but maps to no canonical class is served as its own class**, carrying the raw code and the rule that declined it — never as null, because null is indistinguishable from "not yet loaded" to every consumer. **[as-built 2026-09-03]** five jurisdictions are registered and four carry wells in the spine; Colorado is registered and has not been promoted. The registry landed in v0.76; New Mexico's shared resolver landed in v0.74; the resolver became registry-driven for any jurisdiction whose vocabulary rule declares read-time resolution in v0.78, and the class domain and the served vocabulary in v0.81. The 465-across-59-files prefix count is the code audit's 2026-09-01 measurement and has not been re-taken.
