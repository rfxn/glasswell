...
def test_the_serving_role_has_no_privilege_on_nm_staging(db, staged_spine):
    """SB-01 §3.1.4 holds for the whole schema; this asserts it of the relations D1 created.

    Scoped deliberately: another track's migration grants `glasswell_api` select on four of its
    own staging tables, and a schema-wide assertion here would report that as an NM defect.
    """
    granted = query(
        db,
        "select table_name, privilege_type from information_schema.table_privileges"
        " where grantee = 'glasswell_api' and table_schema = 'staging'"
        " and table_name like 'stg_nm_ocd%'",
    )
