...
describe("class B — the honest-gap register is navigable, and implies nothing (§2.4, §6.5)", () => {
  it("holds twenty-one entries in one exported const", () => {
    // m4's corrected count. The register is curated, not derived — a silent deletion is a
    // gap the product stops admitting to, so the number is asserted rather than measured.
    expect(CLASS_B_DATASETS).toHaveLength(21);
    expect(new Set(CLASS_B_DATASETS.map((entry) => entry.title)).size).toBe(21);
  });
