...
// The three `feat/well-card-2` closes. That branch merges ahead of this one; the fold empties
// this list. Asserted in both directions below, so it can neither be widened quietly nor
// outlive the merge that makes it false.
const EM_DASH_CARRIED = ["src/card/format.ts", "src/chart/chart.ts", "src/map/legend.ts"];
